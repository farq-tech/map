...
import { getMapboxAccessToken, type MapboxBasemap, mapboxStyleUrl, RIYADH_LNG_LAT } from "../../lib/mapboxAccess";
...
export default function FarqMap({
	...
	useEffect(() => {
		if (!token || !containerRef.current || mapRef.current) return; mapboxgl.accessToken = token; const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
		const map = new mapboxgl.Map({ container: containerRef.current, style: mapboxStyleUrl("standard"), center: [20, 18], zoom: reduced ? 11.6 : 1.55, pitch: 0, bearing: 0, projection: "globe", attributionControl: { compact: true } as unknown as boolean, maxPitch: 75, accessToken: token }); mapRef.current = map; let introTimer = 0;
		...
		map.on("style.load", () => { applyBasemap(map); if (introDoneRef.current && mapSession.camera) map.jumpTo(mapSession.camera); });
		map.once("load", () => {
