...
const { queryOpportunities } = require('../lib/map-opportunities-query');
...
function createMapRouter() {
  ...
  router.get('/health', asyncHandler(async (_req, res) => {
    const comparison = await comparisonMap.mapHealth().catch((err) => ({ ok: false, error: err.message }));
  ...
  router.get('/map/places', asyncHandler(async (req, res) => {
  ...
  router.get('/map/opportunities', asyncHandler(async (req, res) => {
    const body = await queryOpportunities({
      bbox: req.query.bbox,
      zoom: req.query.zoom,
      q: req.query.q,
      category: req.query.category,
  ...
  router.get('/map/places/:placeId', asyncHandler(async (req, res) => {
    const place = await comparisonMap.getPlace(req.params.placeId);
    if (!place) return res.status(404).json({ error: 'not_found', place_id: req.params.placeId });
    res.json(place);
  }));

  router.get('/map/neighborhoods', (_req, res) => res.json(emptyCollection()));

  router.get('/neighborhoods/:neighborhoodId/categories/:categoryId', (_req, res) => {
    res.json({
