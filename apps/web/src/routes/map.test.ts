...
describe("map search — shared list/map world", () => {
	...
	it("defaults outdoor paths to map and /compare to the restaurant list", () => {
		expect(resolveMapView({}, "/map")).toBe("map");
		expect(resolveMapView({}, "/")).toBe("map");
		expect(resolveMapView({}, "/compare")).toBe("list");
...
describe("camera in the URL", () => {
	...
	it("keeps restaurant compare on its own path", () => {
		expect(resolveMapProduct("/compare")).toBe("restaurants");
		expect(resolveMapProduct("/")).toBe("outdoor");
		expect(mapStayPath("/compare")).toBe("/compare");
		expect(parseOutdoorLayer("tracks")).toBe("tracks");
		expect(parseOutdoorLayer("burgers")).toBeUndefined();
	});
});
