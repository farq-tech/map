...
export default function MobileMapExperience({ category, q }: { category?: string; q?: string }) {
	...
	const [openDrawer, setOpenDrawer] = useState(false); const [selected, setSelected] = useState<MapOpportunity | null>(null);
	...
	useEffect(() => {
		...
		if (category) qs.set("category", category); if (q) qs.set("q", q); trackFarqMap("map_open", { category: category || "all" });
	...
	return <>
		<div className="farq-mobile-map-experience lg:hidden">
			<div className="farq-mobile-opportunity-button" onClick={() => ordered[0] && selectOpportunity(ordered[0])}>{ordered[0] ? `🔥 فرق ${Math.round(ordered[0].price.difference || 0)} ر.س` : "🔥 اكتشف الفرص حولك"}</div>
		...
		<ExplorationSheet opportunity={selected} mode={mode} index={index + 1} total={ordered.length} onClose={() => setSelected(null)} onNext={nextOpportunity} onCompare={() => { if (!selected?.place.restaurant_id) return; trackFarqMap("compare_clicked", { id: selected.id }); void navigate({ to: "/merchant/$type/$id", params: { type: "restaurant", id: selected.place.restaurant_id } }); }} />
